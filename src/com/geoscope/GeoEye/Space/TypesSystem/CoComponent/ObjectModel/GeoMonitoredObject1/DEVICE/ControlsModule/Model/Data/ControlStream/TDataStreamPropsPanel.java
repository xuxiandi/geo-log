...
public class TDataStreamPropsPanel extends Activity {
	...
	//.
	private TObjectModel.TControlChannelStatus[] 	DataStreamChannelsStatus = null;
	//.
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            //.
            setContentView(R.layout.controlsmodule_datastream_props_panel);
            //.
        	...
        	if (resultCode == RESULT_OK) {
            	try {
            		...
            		if (lvChannels_SelectedIndex >= 0) {
                    	...
                    	//.
                    	final TDestinationStreamChannel DestinationChannel = com.geoscope.GeoLog.DEVICE.ControlsModule.Model.Data.ControlStream.Channels.TDestinationChannelsProvider.Instance.GetChannel(Channel.GetTypeID());
                    	if (DestinationChannel != null) {
                        	DestinationChannel.ID = Channel.ID;
                        	//.
                    					...
                    					if (ObjectModelID != 0) {
                    						...
                    						if (ObjectModel != null) 
                    							try {
                    								...
                    								//.
                    								ObjectModel.Controls_Channel_SetProfile(DestinationChannel.ID, ProfileData); 
                    							}
                    							finally {
                    								ObjectModel.Destroy();
                    							}								
				    							...
				    							synchronized (GSOC) {
				    								...
				    								try {
					    								...
					    								try {
					        								...
					        								DataStreamChannelsStatus = ObjectModel.Controls_Channels_GetStatus(ChannelIDs); 
					    								}
				    									finally {
						    								GSOC.Disconnect();
				    									}
    ...
    private boolean OpenChannelProfile(int Idx) throws Exception {
		...
		TAsyncProcessing Processing = new TAsyncProcessing(this,getString(R.string.SWaitAMoment)) {
					...
					if (ObjectModelID != 0) {
						...
						if (ObjectModel != null) 
							try {
								...
								ObjectModel.SetObjectController(GSOC, true);
								//.
								DestinationChannel.Profile.FromByteArray(ObjectModel.Controls_Channel_GetProfile(DestinationChannel.ID)); 
							}
							finally {
								ObjectModel.Destroy();
							}								
			...
			@Override 
			public void DoOnCompleted() throws Exception {
		    	Intent ProfilePanel = DestinationChannel.Profile.GetProfilePanel(TDataStreamPropsPanel.this);
