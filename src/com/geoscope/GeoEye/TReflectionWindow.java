...
Я боюсь, что сыт по горло древнерусской тоской...
	 ...
	 */
	{
		...
		if (X_B != 0.0) {
			...
			if ((((-X_B) > 0.0) && ((diffXCX0) < 0.0))
					|| (((-X_B) < 0.0) && ((diffXCX0) > 0.0)))
				X_L = -X_L;
		} else {
			YC = (Y_B * Y + Y_A * (X + X_D / X_A)) / (Y_B - (X_B * Y_A / X_A));
			diffYCY0 = YC - Y0;
			X_L = Math.sqrt(Math.pow(diffXCX0, 2) + Math.pow(diffYCY0, 2));
			if (((X_A > 0.0) && ((diffYCY0) < 0.0))
					|| ((X_A < 0.0) && ((diffYCY0) > 0.0)))
				X_L = -X_L;
		}
		...
		if (Y_B != 0.0) {
			...
			if ((((-Y_B) > 0.0) && ((diffXCX0) < 0.0))
					|| (((-Y_B) < 0.0) && ((diffXCX0) > 0.0)))
				Y_L = -Y_L;
		} else {
			YC = (X_B * Y + X_A * (X + Y_D / Y_A)) / (X_B - (Y_B * X_A / Y_A));
			diffYCY0 = YC - Y0;
			Y_L = Math.sqrt(Math.pow(diffXCX0, 2) + Math.pow(diffYCY0, 2));
			if (((Y_A > 0.0) && ((diffYCY0) < 0.0))
					|| ((Y_A < 0.0) && ((diffYCY0) > 0.0)))
				Y_L = -Y_L;
		}
	    //.
		TXYCoord C = new TXYCoord();
		C.X = Xmn+X_L/Math.sqrt(Math.pow(X_A,2)+Math.pow(X_B,2))*(Xmx-Xmn);
		C.Y = Ymn+Y_L/Math.sqrt(Math.pow(Y_A,2)+Math.pow(Y_B,2))*(Ymx-Ymn);
		//.
		return C;
	}
