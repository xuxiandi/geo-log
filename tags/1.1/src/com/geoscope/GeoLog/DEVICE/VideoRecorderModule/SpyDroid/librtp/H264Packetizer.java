...
public class H264Packetizer extends AbstractPacketizer {
		...
		while (sum<length) {
			try { 
				...
				if (oldavailable<available) {
					...
					Log.d(LOG_TAG,"latency: "+latency+", buffer: "+bleft);
					Log.d(LOG_TAG,"Delay: "+delay+" available: "+fis.available()+", oldavailable: "+oldavailable);
	...
	private void send(int size, boolean split) {
		...
		if (rsock.isMarked()) {
			...
			Log.d(LOG_TAG,"a: "+available+", av: "+avnal+", nal: "+naluLength+", t: "+tleft+", aver: "+avdelay+", del: "+delay+", res: "+res+", r2: "+(tleft*naluLength)/bleft);
