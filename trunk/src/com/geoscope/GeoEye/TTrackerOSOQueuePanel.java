...
public class TTrackerOSOQueuePanel extends Activity {
        	...
        	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				try {
					...
					if (ComponentValue != null) 
						TTrackerOSOQueuePanel.this.TypedDataFile_Open(ComponentValue.TypedDataFile_GetDescriptor());
				}
				catch (Exception E) {
	...
	public void TypedDataFile_Open(TTypedDataFileDescriptor TypedDataFileDescriptor) {
		try {
			...
			case SpaceDefines.TYPEDDATAFILE_TYPE_Document:
				try {
					if (TypedDataFileDescriptor.DataFormat.toUpperCase(Locale.ENGLISH).equals(".TXT")) {
						//. open appropriate extent
						intent = new Intent();
						intent.setDataAndType(Uri.fromFile(TypedDataFileDescriptor.GetFile()), "text/plain");
					}
				} catch (Exception E) {
					Toast.makeText(TTrackerOSOQueuePanel.this,getString(R.string.SErrorOfPreparingDataFile)+TypedDataFileDescriptor.DataFile,Toast.LENGTH_SHORT).show();
					return; // . ->
				}
				break; // . >
			...
			case SpaceDefines.TYPEDDATAFILE_TYPE_Image:
				try {
					if (TypedDataFileDescriptor.DataFormat.toLowerCase(Locale.ENGLISH).equals("."+TDrawingDefines.FileExtension)) {
			    		intent = new Intent(this, TDrawingEditor.class);
			  		    intent.putExtra("FileName", TypedDataFileDescriptor.GetFile().getAbsolutePath()); 
			...
			case SpaceDefines.TYPEDDATAFILE_TYPE_Audio:
				try {
					//. open appropriate extent
					intent = new Intent();
					intent.setDataAndType(Uri.fromFile(TypedDataFileDescriptor.GetFile()),"audio/*");
				} catch (Exception E) {
					Toast.makeText(TTrackerOSOQueuePanel.this,getString(R.string.SErrorOfPreparingDataFile)+TypedDataFileDescriptor.DataFile,Toast.LENGTH_SHORT).show();
					return; // . ->
				}
			...
			case SpaceDefines.TYPEDDATAFILE_TYPE_Video:
				try {
					//. open appropriate extent
					intent = new Intent();
					intent.setDataAndType(Uri.fromFile(TypedDataFileDescriptor.GetFile()),"video/*");
				} catch (Exception E) {
					Toast.makeText(TTrackerOSOQueuePanel.this,getString(R.string.SErrorOfPreparingDataFile)+TypedDataFileDescriptor.DataFile,Toast.LENGTH_SHORT).show();
					return; // . ->
				}
