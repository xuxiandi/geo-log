...
@SuppressLint("NewApi")
public class CameraStreamerFRAME0 extends Camera {
	...
	private static class TAudioSampleEncoder extends TAACEncoder {
		...
		protected CameraStreamerFRAME0 Streamer;
		//.
		...
		public TAudioSampleEncoder(CameraStreamerFRAME0 pStreamer, int BitRate, int SampleRate, OutputStream pOutputStream) {
	...
	private static class TVideoFrameEncoder extends TH264Encoder {
		...
		public TVideoFrameEncoder(CameraStreamerFRAME0 pStreamer, int FrameWidth, int FrameHeight, int BitRate, int FrameRate, int pInputBufferPixelFormat, OutputStream pOutputStream, OutputStream pIndexOutputStream, OutputStream pTimestampOutputStream) {
	...
	private static class TVideoFrameEncoderServerClient extends TH264EncoderServer.TClient {
		...
		public TVideoFrameEncoderServerClient(CameraStreamerFRAME0 pStreamer, OutputStream pOutputStream, OutputStream pIndexOutputStream, OutputStream pTimestampOutputStream) {
			super(true);
			Streamer = pStreamer;
			MyOutputStream = pOutputStream;
			MyIndexOutputStream = pIndexOutputStream;
	...
	public CameraStreamerFRAME0(TVideoRecorderModule pVideoRecorderModule) {
		...
		try {
			...
			if (flVideo) {
		        ...
		        synchronized (VideoRecorderModule.MediaFrameServer.CurrentFrame) {
		        	VideoRecorderModule.MediaFrameServer.FrameSize = new Rect(0,0, camera_parameters_Video_FrameSize.width,camera_parameters_Video_FrameSize.height);
